# 解决冲突是我们必须面对的问题。
## 什么是冲突？冲突出现的原因？
### 当A在处理某个文件的代码的时候，B也在处理这个文件的代码，`且两者处理同一行`。A先提交后，B在提交，B就会提示`无法提交`，显示如下：
...
## 如何解决冲突？
### 当我们开始编辑文档的时候，我们先自己去拉一个分支，并随便起个名称（演示用，起名`LQ`）
...
### 回到主页面，你就会发现这里多了个分支`LQ`。我们点击这个分支，进入到这个分支下的代码库
...
### 此时我们假设另一位人员已经提交到main分支一个版本了（也就是两个版本出现了冲突），则会提示如下。我们要点击`Compare & pull request`,base选择`main`，compare选择你编辑的分支，在这个例子中是`LQ`。然后点击`Create pull request`。
...
### 进入到下一个页面后，我们点`Resolve Conflicts`。我们发现LQ分支和main分支的区别，我们自己去修改要保留的。然后`Mark as solve`。点击`Commit merge`。
...
### 返回上个界面，我们点`Merge pull request`后，再点`Confirm merge`。这样冲突就解决了（LQ分支和main分支内容就完全一样了）
